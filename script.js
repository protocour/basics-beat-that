...
var countScore = function () {
  while ((scoreCounter < p1DiceArray.length, scoreCounter++)) {
...
var main = function (input) {
  ...
  if (input == 1 && player == 1) {
    ...
    console.log("p1results", p1Results);
  ...
  // run for player 2
  if (input == 0 && p1DiceArray.length != 0) {
    player = 2;
    input = input;
    dice1 = rollDice();
    dice2 = rollDice();
  ...
  if (input == 1 && player == 2) {
    ...
    myOutputValue = `Player 2, you chose Dice 1 first. Your number is ${p2Results}. Click submit to see who wins.`;
  }
  if (input == 2 && player == 2) {
